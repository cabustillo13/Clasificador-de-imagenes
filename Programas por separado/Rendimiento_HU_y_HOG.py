...
def extraccion(image):
    ...
    hog_image = histograma_hog(aux)
    media, desviacion = estadistica(hog_image)
    ...
    "Para 3 elementos"
    #PARA MOMENTOS DE HU
    return aux, [hu[0], hu[1], hu[3], media, desviacion]
    #PARA HARALICK
...
def entrenamiento_kmeans(datos):
    ...
    tornillo_len = [0, 0, 0, 0, 0]
    tuerca_len = [0, 0, 0, 0, 0]
    arandela_len = [0, 0, 0, 0, 0]
    clavo_len = [0, 0, 0, 0, 0]
    ...
    while (iter < 20):
        ...
        tornillo_mean[3] = sum_tornillo[3] / len(tornillo_datos)
        tornillo_mean[4] = sum_tornillo[4] / len(tornillo_datos)
        ...
        tuerca_mean[3] = sum_tuerca[3] / len(tuerca_datos)
        tuerca_mean[4] = sum_tuerca[4] / len(tuerca_datos)
        ...
        arandela_mean[3] = sum_arandela[3] / len(arandela_datos)
        arandela_mean[4] = sum_arandela[4] / len(arandela_datos) 
        
        clavo_mean[0] = sum_clavo[0] / len(clavo_datos)
        clavo_mean[1] = sum_clavo[1] / len(clavo_datos)
        clavo_mean[2] = sum_clavo[2] / len(clavo_datos)
        clavo_mean[3] = sum_clavo[3] / len(clavo_datos)
        clavo_mean[4] = sum_clavo[4] / len(clavo_datos)
